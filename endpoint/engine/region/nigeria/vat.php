...
// Function to calculate VAT (default rate: 7.5%)
function calculateVAT($unit_price, $quantity, $vat_rate = 0.075) {
...
function generateUniqueInvoiceNumber($pdo) {
    do {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM nigeria_vat_results WHERE Invoice = :Invoice");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Prepare SQL to insert data into nigeria_vat_results table
        $stmt = $pdo->prepare("
            INSERT INTO nigeria_vat_results (
                Username, 
                Invoice, 
                date_generated, 
                due_date, 
    ...
    function insertTaxOverview($pdo, $username, $tax_type, $status, $activity, $payroll, $invoice, $report) {
        ...
        insertTaxOverview($pdo, $user, "Nigeria VAT", $status, $activity, $payroll, $invoice, $report);
}
?>
