...
function generateUniqueInvoiceNumber($pdo) {
    do {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM egypt_vat_results WHERE Invoice = :Invoice");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Prepare SQL to insert data into egypt_vat_results table
        $stmt = $pdo->prepare("
            INSERT INTO egypt_vat_results (
                Username, 
                Invoice, 
                date_generated, 
                due_date, 
