...
// Function to calculate VAT (default rate: 15%)
function calculateVAT($unit_price, $quantity, $vat_rate = 0.15) {
...
function generateUniqueInvoiceNumber($pdo) {
    do {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM southafrica_vat_results WHERE Invoice = :Invoice");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Prepare SQL to insert data into southafrica_vat_results table
        $stmt = $pdo->prepare("
            INSERT INTO southafrica_vat_results (
                Username, 
                Invoice, 
                date_generated, 
                due_date, 
